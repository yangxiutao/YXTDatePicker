...
#define YXTDatePicker_Width  [UIScreen mainScreen].bounds.size.width/3
...
//static CGFloat const kHooDatePickerScrollViewDaysWidth = 200.f;
//
...
static CGFloat const kHooDatePickerScrollViewLeftMargin = 0.5f;
...
#define kHooDatePickerHighlightColor [UIColor blackColor]//[UIColor colorWithRed:255.0/255.0 green:180.0/255.0 blue:18.0/255.0 alpha:1.0]
...
@interface YXTDatePicker ()<UIScrollViewDelegate> {
    // Lines :
...
- (void)setupControl {
    ...
    if (self.datePickerMode == HooDatePickerModeDate) {
        [self buildSelectorYearsOffsetX:0.0 andWidth:YXTDatePicker_Width];
        [self buildSelectorMonthsOffsetX:(_scrollViewYears.frame.size.width + kHooDatePickerScrollViewLeftMargin) andWidth:YXTDatePicker_Width];
...
- (UIView *)headerView {
    if (!_headerView) {
        ...
        cancelButton.frame = CGRectMake(kHooDatePickerPadding, 0.0, kHooDatePickerButtonHeaderWidth, kHooDatePickerHeaderHeight);
        ...
        sureButton.frame = CGRectMake(self.frame.size.width - kHooDatePickerButtonHeaderWidth - kHooDatePickerPadding, 0.0, kHooDatePickerButtonHeaderWidth, kHooDatePickerHeaderHeight);
